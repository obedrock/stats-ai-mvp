...
<objective>
Define all type contracts for the Phase 2 data pipeline: Pydantic schemas (backend), TypeScript interfaces (frontend), and the Zustand analysis store. These are the interfaces that Plans 04-08 implement against.
...
<task type="auto">
  ...
  <action>
    ...
    IMPORTANT: SourceOverride includes a `source` field (pattern "^(FRED|YAHOO)$") so the override endpoint can determine whether to call validate_series() (FRED) or do a basic non-empty check (YAHOO). This addresses the key_links checker issue.

    All models use Pydantic v2 syntax with `model_config` where needed.
  </action>
  <verify>
    <automated>cd backend && uv run python -c "from app.schemas.data import PromptSubmit, ParsedSource, FrequencyConflict, DataPreview, ColumnMapping, UploadResult, AssumptionItem, FetchRequest, SourceOverride; so = SourceOverride(index=0, series_id='GDPC1', source='FRED'); print('source field:', so.source); print('All schemas imported OK')"</automated>
  </verify>
  <done>All Pydantic schemas for the data pipeline API are defined and importable, including source field on SourceOverride</done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    IMPORTANT: SourceOverride includes a `source` field matching the backend schema.
  ...
  <done>TypeScript types mirror backend schemas (including SourceOverride.source); Zustand store manages full pipeline state with persisted mode preference</done>
</task>

</tasks>

<verification>
- `cd backend && uv run python -c "from app.schemas.data import PromptSubmit, ParsedSource, FrequencyConflict, DataPreview, SourceOverride; print('OK')"` exits 0
- `cd frontend && npx tsc --noEmit` exits 0
- frontend/src/store/analysis.ts exports `useAnalysisStore` with mode persistence
</verification>

<success_criteria>
All data pipeline type contracts defined in both backend (Pydantic) and frontend (TypeScript). SourceOverride includes source field for validation routing. Zustand store ready for UI integration. Subsequent plans import these types directly without defining their own.
</success_criteria>
